...
def suma_cubo_pares_for(nums: Iterable[int]) -> int:
    ...
    for ele in alCubo:
        if ele % 2 == 0:
            alCuboPares.append(ele)
    return sum(alCuboPares)
...
def suma_cubo_pares_sum_list(nums: Iterable[int]) -> int:
    ...
    """
    return sum([ele ** 3 for ele in nums if ele ** 3 % 2 == 0])
...
def suma_cubo_pares_sum_gen(nums: Iterable[int]) -> int:
